...
Figure * Board::getFigure(int row, int column) {
	return board[row][column].getFigure();
}

bool Board::setFigure(int row, int column, Figure * figure) {
	if (row > -1 && row < GlobalVaribles::SIZE && column >-1 && column < GlobalVaribles::SIZE && figure != nullptr) {
...
bool Board::makeTileEmpty(int row, int column) {
	if (row > -1 && row < GlobalVaribles::SIZE && column >-1 && col < GlobalVaribles::SIZE) {
